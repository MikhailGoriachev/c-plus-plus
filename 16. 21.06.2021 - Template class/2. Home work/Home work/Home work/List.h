...
// шаблонный класс для представления односвязного списка
template <typename T> 
class List
{
...
public:
    ...
    List(const List& list) : head_(), counter_() {		
		Node<T>* node = list.head_;  // адрес первого элемента из списка lisGoods
    ...
    // провера списка на пустоту
    bool isEmpty() const { return counter_ == 0; }

	// перегрузка операции вывода дружественной шаблоннй функцией
