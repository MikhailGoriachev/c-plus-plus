...
// Удаление элемента из массива по индексу index
Figures &Figures::operator|(int index)
{
	...
	// временный массив - тут будут все элементы исходного, кроме удаляемого элемента
    Triangle *temp = new Triangle[result->length_-1];

	// i - индексирование по исходному массиву
	// k - индексирование по временному массиву
    for (int i = 0, k = 0; i < result->length_; i++) {
        // не копировать во вспомогательный массив удаляемый элемент
        if (index == i) continue;

        // копируем остальные элементы во вспомогательный массив
        temp[k++] = result->triangles_[i];
    } // for i

    result->setData(temp, result->length_ - 1);
    delete[] temp;
