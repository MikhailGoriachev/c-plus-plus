...
int main()
{
    init(L"Домашнее задание на 10.06.2021");
	...
	enum Commands {
		...
		CMD_OUT_TABLE_COLOR,
		// Удаление первого самолета из массива 
		CMD_DELETE_FIRST_PLANE,
		// Удаление самолета из массива по числовому индексу
		CMD_DELETE_PLANE_BY_INDEX,
		// Удаление самолета из массива по регистрационному номеру
		CMD_DELETE_PLANE_BY_NUMBER,
		// Вставка по заданному индексу самолета
		CMD_INSERT_PLANE_BY_INDEX
	}; 
	...
	MenuItem items[N_MENU] = {
		...
		MenuItem("Удаление первого самолета из массива", CMD_DELETE_FIRST_PLANE),
		MenuItem("Удаление самолета из массива по числовому индексу", CMD_DELETE_PLANE_BY_INDEX),
		MenuItem("Удаление самолета из массива по регистрационному номеру", CMD_DELETE_PLANE_BY_NUMBER),
		MenuItem("Вставка по заданному индексу самолета", CMD_INSERT_PLANE_BY_INDEX),
	...
	while (true)
	{
		try {
			...
			case CMD_OUT_TABLE_COLOR:
				...
				// Удаление первого самолета из массива
			case CMD_DELETE_FIRST_PLANE:
				app.deleteFirstPlane();
				break;

				// Удаление самолета из массива по числовому индексу
			case CMD_DELETE_PLANE_BY_INDEX:
				app.deletePlaneByIndex();
				break;

				// Удаление самолета из массива по регистрационному номеру
			case CMD_DELETE_PLANE_BY_NUMBER:
				app.deletePlaneByNumber();
				break;

				// Вставка по заданному индексу самолета
			case CMD_INSERT_PLANE_BY_INDEX:
				app.insertPlaneByIndex();
