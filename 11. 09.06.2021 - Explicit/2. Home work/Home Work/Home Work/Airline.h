...
#pragma region Конструкторы и деструктор
	...
	explicit Airline() :nameCompany_(new char[NAME_COMP_SIZE] {0}), planes_(nullptr), countPlane_() {}

	// констурктор инициализирующий
	explicit Airline(const char* nameCompany, int countPlane);

	// констурктор инициализирующий, принимающий массив самолётов
	explicit Airline(const char* nameCompany, Plane* planes, int countPlane);
...
#pragma region Пергрузка операторов
	...
	Airline& operator()(int index, Plane& newPlane);
