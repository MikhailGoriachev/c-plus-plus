...
int main()
{
    init(L"Домашнее задание на 31.06.2021");
	...
	MenuItem items[N_MENU] = {
		MenuItem("Сохранение и загрузка данных из бинарного файла", CMD_SAVE_ON_READ),
		MenuItem("Сохранение очереди для одного местера в отдельный файл", CMD_SAVE_REPAIRER),
	...
	while (true)
	{
		try {
			...
			{
				// Сохранение и загрузка данных из бинарного файла
			case CMD_SAVE_ON_READ:
				app.saveOnRead();
				break;
				// Сохранение очереди для одного местера в отдельный файл
			case CMD_SAVE_REPAIRER:
				app.saveRepairer();
				break;
			}
