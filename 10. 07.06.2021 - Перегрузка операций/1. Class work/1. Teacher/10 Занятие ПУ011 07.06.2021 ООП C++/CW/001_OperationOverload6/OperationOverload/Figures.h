...
class Figures
{
    ...
    void quickSort(int start, int end, int compare(const Triangle& tr1, const Triangle& tr2));

    // static - для доступности укащатиеля на функцию - для возможности передачи
	// в качестве параметра функции
	static int compareByArea(const Triangle &tr1, const Triangle &tr2);
...
public:
    ...
    Figures operator%(int compare(const Triangle &tr1, const Triangle &tr2));
