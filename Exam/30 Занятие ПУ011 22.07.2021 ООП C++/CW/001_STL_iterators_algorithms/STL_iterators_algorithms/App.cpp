...
void App::iteratorsDemo()
{
	...
	*os_stream = 40'000;  // вывод
    ++os_stream;          // продвижение по выводу
    *os_stream = 42;
    ++os_stream;
...
// binary_function<bool, int, double>      double foo(bool, iint);
// 
...
void App::algorithmsDemo()
{
    ...
    generate(v1.begin(), v1.end(), Random(-2, 2));
    cout << "    Новый вектор\":\n    ";
    for_each(v1.begin(), v1.end(), Output());
    cout << "\n";
	
	// accumulate() - суммирование элементов вектора
	// pred_res = init; for (x: vector)  pred_res = op(pred_res, x) -->  pred_res += x 
	// начало интервала суммирования, конец интервала суммирования, начальное значение
    int sum = accumulate(v1.begin(), v1.end(), 0);
    cout << "    Сумма элементов вектора: " << setw(3) << sum << "\n";
    
    generate(v1.begin(), v1.end(), Random(-5, 5));
    cout << "\n    Вектор для произведения:\n";
    for_each(v1.begin(), v1.end(), Output());

	cout << "\n    Произведение: "; // при помощи стандартного функтора!!!!
    long long int mult = accumulate(v1.begin(), v1.end(), 1, multiplies<int>());
    cout << mult << "\n";
    
    // расчет произведения нечетных чисел при помощи собственного функтора
    class OddMultiplies: public binary_function<int, int, int>
    {
    public:
        int operator()(int predResult, int item)
        {
            return predResult * ((item & 1) != 0 ? item : 1);
        }
    };
    mult = accumulate(v1.begin(), v1.end(), 1, OddMultiplies());
    cout << "    Произведение нечетных: " << mult << "\n";


	// сумма положительных
	class SumPositives: public binary_function<int, int, int>
	{
	public:
		int operator()(int predRes, int x)
		{
            return predRes + (x >= 0 ? x : 0);
		}
	};
    sum = accumulate(v1.begin(), v1.end(), 0, SumPositives());
    cout << "    Сумма положительных: " << sum << "\n";
    
	// алгоритмы count(), count_if()
	// количество положительных
	class IsPositive: public unary_function<int, bool>
	{
	public:
        bool operator()(int x) { return x >= 0; }
	};
    int positives = count_if(v1.begin(), v1.end(), IsPositive());
    cout << "    Количество положительных: " << positives << "\n\n";
    
	// алгоритм find()
    // поиск значения
	auto it = find(v1.begin(), v1.end(), v1[5] /*getRand(-10, 10)*/);
    if (it != v1.end())
        cout << "\n    :)  " << *it << "\n";
    else
        cout << "\n    :(\n";
	...
	// алгоритм find_if()
	// поиск первого элемента, удовлетворяющего некоторому условию
	// например, поиск первого положительного
    it = find_if(v1.begin(), v1.end(), IsPositive());
    cout<< (it != v1.end() ? "    Первый положительный "s + to_string(*it) : "в векторе нет положительных")
        << "\n" << string(72, '—') << "\n\n";
} // App::algorithmsDemo