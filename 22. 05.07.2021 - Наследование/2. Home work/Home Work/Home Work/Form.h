...
#pragma region Методы
	...
	// перегрузка операции меньше по фамилии 03.07.2021
	bool operator<(const Form& form) { return strcmp(surname_, form.surname_) < 0; }

	// перегрузка операции меньше или равно по фамилии 03.07.2021
	bool operator<=(const Form& form) { return strcmp(surname_, form.surname_) <= 0; }

	// перегрузка операции равно по фамилии 03.07.2021
	bool operator==(const Form& form) { return strcmp(surname_, form.surname_) == 0; }

	// перегрузка операции НЕравно по фамилии 03.07.2021
	bool operator!=(const Form& form) { return strcmp(surname_, form.surname_) != 0; }

	// перегрузка операции больше или равно по фамилии 03.07.2021
	bool operator>=(const Form& form) { return strcmp(surname_, form.surname_) >= 0; }

	// перегрузка операции больше по фамилии 03.07.2021
	bool operator>(const Form& form) { return strcmp(surname_, form.surname_) > 0; }

	// вывод шапки таблицы
	static const char* showHead()
	{
		return "\t+————+——————————————————————+——————————————————————+——————————————————————+————————+—————————+————————————+\n"
	...
	static const char* showLine()
	{
		return "\t+————+——————————————————————+——————————————————————+——————————————————————+————————+—————————+————————————+\n";
	}

	// вывод элемента в цвете для таблицы
	void showColor(int i = 1, short cl = WHITE_ON_BLACK)
	{
		cout << "\t| " << color(CYAN_ON_BLACK) << setw(2) << i++
			<< left << color(cl) << " | " << setw(Form::SURNAME_MAX_SIZE - 30)
			<< color(YELLOW_ON_BLACK) << surname_
			<< color(cl) << " | "
			<< setw(Form::NAME_MAX_SIZE - 20)
			<< color(YELLOW_ON_BLACK) << name_
			<< color(cl) << " | "
			<< setw(Form::PATRONYMIC_MAX_SIZE - 30)
			<< color(YELLOW_ON_BLACK) << patronymic_
			<< color(cl) << " | " << right
			<< setw(6)
			<< color(GREEN_ON_BLACK) << countFamily_
			<< color(cl) << " |   "
			<< setw(4)
			<< color(GREEN_ON_BLACK) << area_
			<< color(cl) << " | "
			<< setw(15)
			<< color(GREEN_ON_BLACK) << date_
			<< color(cl) << " |\n";
	}
	...
	void write(FILE* file)
	{
		...
		fwrite(surname_, sizeof(*surname_), SURNAME_MAX_SIZE, file);
		fwrite(name_, sizeof(*name_), NAME_MAX_SIZE, file);
		fwrite(patronymic_, sizeof(*patronymic_), PATRONYMIC_MAX_SIZE, file);
		fwrite(&countFamily_, sizeof(countFamily_), 1, file);
		fwrite(&area_, sizeof(area_), 1, file);
		fwrite(&countRoom_, sizeof(countRoom_), 1, file);
		fwrite(&date_, sizeof(date_), 1, file);
	}
	...
	void read(FILE* file)
	{
		...
		fread(surname_, sizeof(*surname_), SURNAME_MAX_SIZE, file);
		fread(name_, sizeof(*name_), NAME_MAX_SIZE, file);
		fread(patronymic_, sizeof(*patronymic_), PATRONYMIC_MAX_SIZE, file);
		fread(&countFamily_, sizeof(countFamily_), 1, file);
		fread(&area_, sizeof(area_), 1, file);
		fread(&countRoom_, sizeof(countRoom_), 1, file);
		fread(&date_, sizeof(date_), 1, file);
	}
